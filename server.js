  ...
  fs.writeFile(inputPath, code, (writeErr) => {
    ...
    execFile(
      ...
      (err, stdout, stderr) => {
        ...
        if (err) {
          // Also try to clean up output file if it exists
        ...
        // Read the obfuscated output file
        fs.readFile(outputPath, "utf8", (readErr, data) => {
          // Clean up output file regardless
          fs.unlink(outputPath, () => {});
    ...
    <section class="section" id="sec-howto">
      ...
      <div class="prose">
        <ol>
          ...
          <li>Copy the result from the <b>Output</b> box or download it as a .lua file.</li>
        </ol>
    ...
    <section class="section" id="sec-apidocs">
      ...
      <div class="prose">
        ...
        <ul>
          ...
          <li><b>Web dashboard:</b> ${WEB_LIMIT} requests per day per IP.</li>
